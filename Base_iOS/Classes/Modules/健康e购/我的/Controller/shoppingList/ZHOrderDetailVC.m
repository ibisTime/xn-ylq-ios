...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"订单详情";
    UITableView *tableV = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kSuperViewHeight) style:UITableViewStylePlain];
...
//    }];
    ...
    payVC.rmbAmount = @([rmb longLongValue]*[detailModel.quantity longLongValue]); //把人民币传过去
...
//    payVC.postage = self.postage;
    ...
    payVC.paySucces = ^(){
        ...
        for (UIViewController *vc in self.navigationController.viewControllers) {
            ...
            if ([vc isKindOfClass:[ZHShoppingListVC class]]) {
...
//    payVC.amoutAttr = [TLCurrencyHelper totalPriceAttr2WithQBB:self.order.amount3 GWB:self.order.amount2 RMB:self.order.amount1];
//    
//        payVC.paySucces = ^(){
//            
