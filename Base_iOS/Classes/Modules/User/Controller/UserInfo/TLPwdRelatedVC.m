...
- (void)setUpUI {
    ...
    //确认按钮
    UIButton *confirmBtn = [UIButton buttonWithTitle:btnStr titleColor:kWhiteColor backgroundColor:kAppCustomMainColor titleFont:15.0 cornerRadius:45/2.0];
    
    confirmBtn.frame = CGRectMake(15, rePwdTf.yy + 30, kScreenWidth - 30, 45);
...
- (void)confirm {
    ...
    if (self.type == TLPwdTypeTradeReset) { //支付密码
    ...
    [http postWithSuccess:^(id responseObject) {
        ...
        if(self.type == TLPwdTypeForget) {
            ...
            [TLUser user].tradepwdFlag = [NSNumber numberWithBool:YES];
