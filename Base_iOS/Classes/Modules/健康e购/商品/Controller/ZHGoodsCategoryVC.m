...
- (void)viewDidLoad {
    ...
    //上部
    UIScrollView *smallChooseView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 40)];
    ...
    //下部
    self.switchScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, self.smallChooseScrollView.yy, kScreenWidth, kScreenHeight - kNavigationBarHeight - kTabBarHeight - smallChooseView.yy)];
    ...
    self.switchScrollView.contentSize = CGSizeMake(kScreenWidth * count, self.switchScrollView.height);
    ...
    [self.smallCategories enumerateObjectsUsingBlock:^(ZHCategoryModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        //---
        UIButton *btn = [UIButton buttonWithTitle:obj.name titleColor:[UIColor textColor] backgroundColor:kWhiteColor titleFont:13];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if ([self.isHaveChildVC[index] isEqualToNumber:@1]) {
        ...
        defaultVC2.view.frame = CGRectMake(kScreenWidth*index, 0, self.switchScrollView.width, self.switchScrollView.height);
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset {
    ...
    if ([scrollView isEqual:self.smallChooseScrollView]) {
        ...
        [currentBtn setTitleColor:[UIColor themeColor] forState:UIControlStateNormal];
        [self.lasBtn setTitleColor:[UIColor textColor] forState:UIControlStateNormal];
...
- (void)smallChoose:(UIButton *)btn {
    ...
    //下部改变
    [self.switchScrollView setContentOffset:CGPointMake(kScreenWidth * (btn.tag - 1000), 0) animated:NO];
